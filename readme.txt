...
primitives data types Link
variables, string, numbers, booleans, undefined, null, NaN Link
arithmetic operators Link
comparison operators Link
logical operators Link
conditional statements (if else, switch) Link
functions, arguments, return statement, scope Link
callbacks Link, higher order functions Link
for loops, while, do while Link
Arrays Link
declaration
accessing items
assignment
...
// console.log( rec(arr) )
DOM manipulation Link, another Link
createElement
append
getElementById
querySelector
...
setAttribute
children methods
event API, eventListener Link
event bubbling Link
event capturing Link
style and attribute manipulation with DOM Link
JSON Link
using AJAX / XHR Link
callback, callback hell Link
...
task/event queue
WEB API Link
promises Link OR Link
promise chaining
advantage over callbacks
async, await Link OR Link
fetch API Link
axios Link
DOCS for Above: Link
...
Sets, Map Link
modules Link
localStorage, Link , cookie Link, sessionStorage Link
ECMAScript Link, TC39 Link
polyfill Link
tree shaking Link OR Link
HTTP protocols Link
